...
function lgraph = residualCIFARlgraph(netWidth,numUnits,unitType)
...
layers = [
    imageInputLayer([16 64 18],'Name','input')
    convolution2dLayer(3,netWidth,'Padding','same','Name','convInp')
    batchNormalizationLayer('Name','BNInp')
    reluLayer('Name','reluInp')];

% Stage one. Activation size is 64-by-64.
...
% Stage two. Activation size is 32-by-32.
for i = 1:unitsPerStage
    if i==1
        stride = 2;
    else
...
% Stage three. Activation size is 16-by-16
for i = 1:unitsPerStage
    if i==1
        stride = 2;
    else
...
% Stage four. Activation size is 8-by-8
for i = 1:unitsPerStage
    if i==1
        stride = 2;
    else
    ...
    layers = [layers
        convolutionalUnit(4*netWidth,stride,['S4U' num2str(i) '_'])
        additionLayer(2,'Name',['add4' num2str(i)])
        reluLayer('Name',['relu4' num2str(i)])];
end

% Output section.
layers = [layers
    averagePooling2dLayer([2 8],'Name','globalPool')
    fullyConnectedLayer(10,'Name','fcFinal1')
    fullyConnectedLayer(1,'Name','fcFinal2')
...
for i = 1:unitsPerStage-1
    ...
    lgraph = connectLayers(lgraph,['relu4' num2str(i)],['add4' num2str(i+1) '/in2']);
end
...
skip3 = [convolution2dLayer(1,numF,'Stride',2,'Name','skipConv3')
    batchNormalizationLayer('Name','skipBN3')];
lgraph = addLayers(lgraph,skip3);
lgraph = connectLayers(lgraph,['relu3' num2str(unitsPerStage)],'skipConv3');
