...
Trail_numH = [57 55 60 56 56 14 49 59 58 57 56 54 58 45 59];
Trail_numD = load('.\ScoringDepression_nback\Trail_back1_numD.mat');
Trail_numD = Trail_numD.Trail_back1_numD;
...
indicesD = crossvalind('Kfold', Trail_numD, Kfold);
    ...
    EEGDataD.EEGdata_back1 = reshape(EEGDataD.EEGdata_back1(:,:,1:64*18), [size_EEGDataD(1), size_EEGDataD(2), 64, 18]);
%         EEGDataD.EEGdata_back1 = permute(EEGDataD.EEGdata_back1, [2 3 4 1]);
    EEGDataH.EEGdata_back1 = reshape(EEGDataH.EEGdata_back1(:,:,1:64*18), [size_EEGDataH(1), size_EEGDataH(2), 64, 18]);
%         EEGDataH.EEGdata_back1 = permute(EEGDataH.EEGdata_back1, [2 3 4 1]);

Result_all_back1 = cell(4,Kfold);
for k = 1:Kfold
    ...
    for iH = 1: length(testH)
        ...
        Data_temp = EEGDataH.EEGdata_back1(from_H:to_H,:,:,:); 
        if testH(iH) == 0
    ...
    for iD = 1: length(testD)
        ...
        Data_temp = EEGDataD.EEGdata_back1(from_D:to_D,:,:,:); 
        if testD(iD) == 0
...
%         grid on;
    ...
    Result_all_back1(1,k) = {Trail_numH};
    Result_all_back1(2,k) = {accuracy_H};
    Result_all_back1(3,k) = {Trail_numD};
    Result_all_back1(4,k) = {accuracy_D};
end

save Result_all_back1 Result_all_back1
