...
Trail_numD = load('.\ScoringDepression_nback\Trail_back0_numD.mat');
Trail_numD = Trail_numD.Trail_back0_numD;
...
%         EEGDataD.EEGdata_back0 = permute(EEGDataD.EEGdata_back0, [2 3 4 1]);
    EEGDataH.EEGdata_back0 = reshape(EEGDataH.EEGdata_back0(:,:,1:64*18), [size_EEGDataH(1), size_EEGDataH(2), 64, 18]);
%         EEGDataH.EEGdata_back0 = permute(EEGDataH.EEGdata_back0, [2 3 4 1]);

Result_all_back0 = cell(2,Kfold);
for k = 1:Kfold
    ...
    for iH = 1: length(testH)
        ...
        Data_temp = EEGDataH.EEGdata_back0(from_H:to_H,:,:,:); 
        if testH(iH) == 0
    ...
    for iD = 1: length(testD)
        ...
        Data_temp = EEGDataD.EEGdata_back0(from_D:to_D,:,:,:); 
        if testD(iD) == 0
...
%         grid on;
    ...
    Result_all_back0(1,k) = {Trail_numH};
    Result_all_back0(2,k) = {accuracy_H};
    Result_all_back0(3,k) = {Trail_numD};
    Result_all_back0(4,k) = {accuracy_D};
end

save Result_all_back0 Result_all_back0
